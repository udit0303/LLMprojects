...
# Note: Anthropic does not provide an embeddings API, so we use OpenAI
# embeddings alongside the Claude chat LLM on the client side.
...
def ingest_document(file_path: str) -> str:
    ...
    embeddings using OpenAI's model, and stores them in a persistent
    ...
    try:
        # 2. Initialize the embedding model (OpenAI; Anthropic offers no embeddings).
...
def query_rag_store(query: str) -> str:
    ...
    try:
        ...
        # required for similarity search to work against the stored vectors.
        embeddings = _get_embeddings()
