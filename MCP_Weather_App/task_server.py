...
@mcp.tool()
def remove_task(task_description: str) -> str:
    """
    Removes a task from the persistent task list file by matching the description.

    Args:
        task_description: The description of the task to remove. A partial match is sufficient.

    Returns:
        A string confirming the task was removed, or a message if it wasn't found.
    """
    if not os.path.exists(TASKS_FILE):
        return "No tasks file found. There are no tasks to remove."

    try:
        with open(TASKS_FILE, "r") as f:
            tasks = [line.strip() for line in f.readlines() if line.strip()]

        matching = [t for t in tasks if task_description.lower() in t.lower()]

        if not matching:
            return f"No task matching '{task_description}' was found."

        remaining = [t for t in tasks if task_description.lower() not in t.lower()]

        with open(TASKS_FILE, "w") as f:
            f.write("\n".join(remaining) + ("\n" if remaining else ""))

        removed = ", ".join(f"'{t}'" for t in matching)
        return f"Removed task(s): {removed}."
    except Exception as e:
